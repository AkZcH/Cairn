> **Archived, historical planning document.** This was the original PRD, written before implementation began. Several elements described here (Neo4j knowledge graph, LangGraph agents, LlamaIndex, Streamlit, Chroma) were never built or were deliberately cut. See `docs/PRD.md` for what actually shipped. Kept for reference, not ingested into Cairn's own knowledge base.
